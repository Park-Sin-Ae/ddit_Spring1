...
<body>
	...
	<!-- 
		...
		웹 애플리케이션에서 화면은 웹 브라우저가 랜더링하므로 브라우저가 읽어서 해설할 수 있는 HTML로
		최종 변환 될 수 있도록 작성합니다.
		form.jsp 뷰는 제목, 카테고리, 가격을 입력받을 수 있는 형식을 가진 HTML 입니다.
		
		저장 버튼을 클릭하면 form ~ /form 태그 안의 내용이 서버로 전송됩니다.
		서버로 전달되는 항목들은(input과 같은 데이터 관정 요소 ) form 태그 안에 존재해야 하고,
		name 속성을 가져야합니다.
		서버는 name 속성을 키로, value 속성을 값으로 판단합니다.
		
		post 방식은 http 파라미터라 주소창에 노출되지 않고, 주소창의 변화 없이 데이터만
		서버로 전달할 수 있습니다.
	 -->
	<div class="container">
