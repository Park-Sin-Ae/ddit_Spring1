 ...
 * 해당 어노테이션이 있는 클래스는 스프링이 데이터를 관리하는 클래스라고 인지하여 자바 빈(Java Bean)
 * 으로 등록해서 관리한다.
 * 
 * SqlSessionTemplate 객체를 멤버 변수로 선언하는 이유는 mapper xml을 실행시키기 위해서다
 * 해당 객체 위에 @Inject 또는 @Autowried를 붙여서 SqlSessionTemplate 객체를 사용할 수 
 * 있도록 한다.
 * 
 * SqlSessionTemplate 객체는 new 키워드를 통해 직접 생성하지 않고,
 * 의존석 주입(Dependency Injection - DI)을 통해 주입받는다.
 * 스프링은 미리 만들어 놓은 SqlSessionTemplate 타입 객체를 BookDAO 객체에 주입한다.
 * 해당 과정은 스프링에서 자동 실행되며 개발자가 직접 SqlSessionTemplate 객체를 생성하는
 * 일 없이 곧바로 사용할 수 있다.
 * 
 * SqlSessionTemplate 객체는 root-context.xml 에서 정의해둔 객체이기도 하고,
 * 서버가 시작 될 때 스프링은 미리 xml을 읽어 객체를 인스턴스화 해둔다. (메모리 탑재)
 * 
 */
@Repository
public class BookDAOImpl implements IBookDAO {
	...
	/*
	 * 요청에서부터 DAO까지 map에 title, category, price가 덤겨져서 온다.
	 * 그리고, useGeneratedKeys와 keyProperty의 설정 덕분에 book 테이블의 pk인 book_id
	 * 항목이 새롭게 생성된다. 
	 */
	@Override
	public int insertBook2(Map<String, Object> map) {
		 ...
		 * 전달된 map 객체에 book 테이블의 pk인 book_id 항목이 새롭게 생성된다.
		 * 
		 * [기존 Map] :::
		 * {
		 * 		"title" : "제목",
		 ...
		 * insert 쿼리의 경우 성공하면 1개의 행(row)이 생기므로 1을 리턴하고 실행하면 0을 리턴한다.
		 * 
		 */
